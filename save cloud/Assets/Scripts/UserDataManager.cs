...
public static class UserDataManager
{
    ...
    {
        //Cek apa ada data yang tersimpan sebagai PROGRESS_KEY
        if (!PlayerPrefs.HasKey(PROGRESS_KEY))
        {
            //Jika tidak, maka simpan data baru dan upload ke Cloud
            Save(true);
        }
        else
        {
    ...
    public static IEnumerator LoadFromCloud(System.Action onComplete)
    {
        ...
        const long maxAllowedSize = 1024 * 1024; //Sama dengan 1MB
        ...
        //Jika sukses download, maka simpan data hasil download
        if (isSuccessfull)
        {
            Save();
        }
